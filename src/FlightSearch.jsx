...
function FlightSearch() {
  ...
  const searchFlights = async () => {
    ...
    try {
      ...
      setResult(data);
      setTabIndex(1); // Move to the next tab to display the map
    } catch (error) {
      ...
      {tabIndex === 0 && (
        <Paper sx={{ p: 4, borderRadius: 4, boxShadow: 3, minHeight: "auto" }}>
          ...
          <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
            <TextField fullWidth label="Origin" variant="outlined" value={origin} onChange={(e) => setOrigin(e.target.value)}
          ...
          <Button fullWidth variant="contained" color="primary" onClick={searchFlights} sx={{ py: 1.5, fontSize: "1rem", fontWeight: "bold", mt: 2 }} disabled={loading}>
